...
class MLOpsMonitor:
    ...
    def __init__(self):
        np.random.seed(42)
        self.baseline_amounts = np.random.lognormal(mean=9.2, sigma=1.3, size=1000)
    ...
    def check_drift(self) -> Dict:
        if len(self.current_amounts) < 25:
            sample_curr = np.random.lognormal(mean=9.3, sigma=1.35, size=50)
        else:
    ...
    def trigger_retraining(self, gnn_manager, graph, processor, data_generator) -> Dict:
        df_fresh = data_generator.generate_dataset(num_normal=50, num_rings=3, num_smurfs=3)
